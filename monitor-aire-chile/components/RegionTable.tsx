    ...
    <div className="w-full overflow-hidden rounded-xl border border-[#d4cebe] dark:border-slate-800/60 bg-white/80 dark:bg-slate-950/40 backdrop-blur-md shadow-md dark:shadow-xl">
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs sm:text-sm text-[#4a453c] dark:text-slate-300">
            <thead className="bg-[#e4dec9]/30 dark:bg-slate-900/70 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-[#6e685e] dark:text-slate-400 border-b border-[#d4cebe]/40 dark:border-slate-800/40">
              <tr>
                <th className="w-8 sm:w-12 px-2 sm:px-4 py-2 sm:py-3"></th>
                <th
                  className="cursor-pointer select-none px-2 sm:px-4 py-2 sm:py-3 font-medium transition-colors hover:text-[#2d2a24] dark:hover:text-slate-200"
                  onClick={() => handleSort('region')}
                >
                  Región<span className="hidden sm:inline"> / Localidad</span>
                  <SortIcon active={sortConfig.key === 'region'} direction={sortConfig.direction} />
                </th>
                <th
                  className="cursor-pointer select-none px-2 sm:px-4 py-2 sm:py-3 text-center font-medium transition-colors hover:text-[#2d2a24] dark:hover:text-slate-200 hidden sm:table-cell"
                ...
                >
                  <span className="hidden sm:inline">Peor calidad</span>
                  <span className="sm:hidden">Calidad</span>
                  <SortIcon active={sortConfig.key === 'severity'} direction={sortConfig.direction} />
                </th>
                <th
                  className="cursor-pointer select-none px-2 sm:px-4 py-2 sm:py-3 text-right font-medium transition-colors hover:text-[#2d2a24] dark:hover:text-slate-200"
                  onClick={() => handleSort('activeMax')}
                >
                  <span className="hidden sm:inline">Máx {activePollutant.toUpperCase()}</span>
                  <span className="sm:hidden">{activePollutant.toUpperCase()}</span>
                  <SortIcon active={sortConfig.key === 'activeMax'} direction={sortConfig.direction} />
                </th>
                <th className="px-2 sm:px-4 py-2 sm:py-3 text-center font-medium">Semáforo</th>
              </tr>
            </thead>
                ...
                <tbody key={row.region} className="group/region">
                  ...
                  >
                    <td className="px-2 sm:px-4 py-2 sm:py-3">
                      <div
                        className={`flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-md border border-[#b5ae9b]/60 dark:border-slate-700/50 bg-white/60 dark:bg-slate-900/50 transition-transform duration-300 ${isOpen ? 'rotate-90' : ''
                          }`}
                      >
                        <svg
                          className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-[#6e685e] dark:text-slate-400"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={3}
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3 font-semibold text-[#2d2a24] dark:text-slate-200 max-w-[120px] sm:max-w-none truncate">
                      {row.region}
                    </td>
                    <td className="px-2 sm:px-4 py-2 sm:py-3 text-center tabular-nums text-[#6e685e] dark:text-slate-400 hidden sm:table-cell">
                      {row.stations.length}
                    </td>
                    <td className="px-2 sm:px-4 py-2 sm:py-3">
                      <span
                        className={`inline-flex items-center rounded-full px-2 sm:px-2.5 py-0.5 text-[9px] sm:text-[11px] font-bold uppercase tracking-wide shadow-lg ${getContrastTextColor(row.worstColor)}`}
                        style={{ backgroundColor: row.worstColor }}
                      >
                        {row.worstCategory}
                      </span>
                    </td>
                    <td className="px-2 sm:px-4 py-2 sm:py-3 text-right tabular-nums font-bold text-[#2d2a24] dark:text-slate-200 text-[11px] sm:text-sm">
                      {activePollutant === 'all'
                        ? (
                    ...
                    <td className="px-2 sm:px-4 py-2 sm:py-3 text-center">
                      <span
                        className="inline-block h-3 w-3 sm:h-3.5 sm:w-3.5 rounded-full ring-2 ring-white/10"
                        style={{
